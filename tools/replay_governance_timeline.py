...
try:
    from cgf_policy import PolicyEvaluator, PolicyDecision
...
def main():
    ...
    parser.add_argument(
        "--policy-bundle",
        help="Path to policy bundle for re-evaluation verification"
    )
    parser.add_argument(
        "--verify-mode",
        action="store_true",
        help="Re-evaluate proposal against policy bundle and verify match"
    )
    ...
    if args.verify_mode:
        if not HAS_POLICY_ENGINE:
            print("\nERROR: Policy engine not available. Install cgf_policy package.")
            return
        
        if not args.policy_bundle:
            print("\nERROR: --policy-bundle required for --verify-mode")
            return
        
        if not replay.decision:
            print("\nWARNING: No recorded decision in replay - cannot verify")
            return
        
        print("\n" + "=" * 70)
        print("POLICY VERIFICATION MODE")
        print("=" * 70)
        
        from cgf_policy import PolicyBundle, DecisionType
        
        # Load policy bundle
        try:
            bundle = PolicyBundle.load(args.policy_bundle)
            print(f"Policy Bundle: {bundle.policy_version}")
            print(f"Bundle Hash: {bundle.bundle_hash}")
            print(f"Rules: {len(bundle.rules)}")
        except Exception as e:
            print(f"ERROR: Failed to load policy bundle: {e}")
            return
        ...
        # Re-evaluate
        print(f"\nRe-evaluating proposal {proposal_id}...")
        print(f"  Action Type: {replay.proposal.action_type}")
        print(f"  Risk Tier: {replay.proposal.risk_tier}")
        
        # Extract context and signals (empty if not available in replay)
        context = {}  # Could be extracted from events if available
        signals = {}
        
        new_decision = evaluator.evaluate(replay.proposal, context, signals)
        
        # Compare
        print(f"\nOriginal Decision:")
        print(f"  Decision: {replay.decision.decision.value}")
        print(f"  Confidence: {replay.decision.confidence:.2f}")
        print(f"  Reason Code: {replay.decision.reason_code}")
        
        print(f"\nRe-evaluated Decision:")
        print(f"  Decision: {new_decision.decision.value}")
        print(f"  Confidence: {new_decision.confidence:.2f}")
        print(f"  Reason Code: {new_decision.reason_code}")
        print(f"  Matched Rules: {new_decision.matched_rule_ids}")
        ...
        # Check decision type
        if new_decision.decision.value != replay.decision.decision.value:
            print(f"  ✗ Decision mismatch: {new_decision.decision.value} != {replay.decision.decision.value}")
            match = False
        else:
            print(f"  ✓ Decision match: {new_decision.decision.value}")
        
        # Check confidence (within epsilon)
        confidence_diff = abs(new_decision.confidence - replay.decision.confidence)
        if confidence_diff > 0.001:
            print(f"  ✗ Confidence mismatch: {new_decision.confidence:.3f} vs {replay.decision.confidence:.3f} (diff: {confidence_diff:.3e})")
            match = False
        else:
            print(f"  ✓ Confidence match: {new_decision.confidence:.2f}")
        
        # Check reasons
        if new_decision.reason_code != replay.decision.reason_code:
            print(f"  ⚠ Reason code differs: {new_decision.reason_code} vs {replay.decision.reason_code}")
        else:
            print(f"  ✓ Reason code match: {new_decision.reason_code}")
        
        # Check proposal hash if available
        if replay.decision and hasattr(replay.decision, 'proposal_hash') and replay.decision.proposal_hash:
            if new_decision.proposal_hash == replay.decision.proposal_hash:
                print(f"  ✓ Proposal hash match: {new_decision.proposal_hash}")
            else:
                print(f"  ⚠ Proposal hash differs:")
                print(f"    New: {new_decision.proposal_hash}")
                print(f"    Original: {replay.decision.proposal_hash}")
        
        # Final result
        print(f"\n{'=' * 70}")
        if match:
            print(f"✓ VERIFICATION PASSED")
            print(f"  Original decision matches re-evaluation with policy v{bundle.policy_version}")
        else:
            print(f"✗ VERIFICATION FAILED")
            print(f"  Policy re-evaluation produced different decision")
        print(f"{'=' * 70}")
