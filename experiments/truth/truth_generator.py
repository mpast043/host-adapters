...
class TruthLabelGenerator:
    ...
    Claim taxonomy for host-adapters:
        W02-W20: Observer structure claims
        claim2_seed_perturbation: Seed perturbation regression
        claim3_optionb_regime_check: Option B regime check
    """
    ...
    def generate(self, claim_id: str, substrate_id: Optional[str] = None, seed: Optional[int] = None) -> TruthLabel:
        # Map claim families to plugin modules.
        # W02-W20 are observer structure claims - use analytical truths
        if claim_id.startswith("W") and claim_id[1:3].isdigit():
            from .plugins.observer_claims import generate_truth
            return generate_truth(claim_id, self.root, substrate_id=substrate_id, seed=seed)

        # Claim 2 and 3 are regression tests
        if claim_id.startswith("claim2") or claim_id.startswith("claim3"):
            from .plugins.regression_claims import generate_truth
            return generate_truth(claim_id, self.root, substrate_id=substrate_id, seed=seed)
