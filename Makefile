.PHONY: all install test test-fast lint format clean workflow-auto workflow-auto-once workflow-auto-supervisor workflow-physics-auto workflow-audit openclaw-opt-check local-compute-mcp framework-selection-plan
...
WORKFLOW_CLAIM_MAP_TARGET_UNRESOLVED ?= 0
WORKFLOW_CLAIM_MAP_STALL_CYCLES ?= 2
...
workflow-physics-auto:
	$(PYTHON) tools/workflow_auto_multi_agent.py --repo-root . --artifacts-root "$(DATA_REPO)" --focus-objective B --until-resolved --sleep-seconds $(WORKFLOW_PHYSICS_SLEEP_SECONDS) --research-on-underdetermined --claim-map "$(WORKFLOW_CLAIM_MAP)" --claim-map-target-unresolved $(WORKFLOW_CLAIM_MAP_TARGET_UNRESOLVED) --claim-map-stall-cycles $(WORKFLOW_CLAIM_MAP_STALL_CYCLES) $(if $(filter 1,$(WORKFLOW_REQUIRE_CLAIM_MAP)),--require-claim-map-gate,) $(if $(filter 1,$(WORKFLOW_START_FRESH)),--start-fresh,)
