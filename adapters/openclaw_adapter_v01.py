...
# ============== CONFIGURATION (P0 Fix: env vars) ==============

class Config:
    """Configuration from environment variables (A2 fix)."""
    # Priority: constructor args > env vars > defaults
    DEFAULT_CGF_ENDPOINT = "http://127.0.0.1:8080"
    DEFAULT_TIMEOUT_MS = 500
    
    CGF_ENDPOINT = os.getenv("CGF_ENDPOINT", DEFAULT_CGF_ENDPOINT)
    CGF_TIMEOUT_MS = int(os.getenv("CGF_TIMEOUT_MS", DEFAULT_TIMEOUT_MS))
    
    @classmethod
    def validate(cls) -> bool:
        """Validate configuration."""
        try:
            timeout = int(os.getenv("CGF_TIMEOUT_MS", cls.DEFAULT_TIMEOUT_MS))
            return timeout > 0
        except ValueError:
            return False
...
class OpenClawAdapter:
    ...
    def __init__(
        self,
        cgf_endpoint: Optional[str] = None,  # A2: explicit constructor arg
        adapter_type: str = "openclaw",
        host_config: Optional[Dict] = None,
        timeout_ms: Optional[int] = None  # A2: explicit constructor arg
    ):
        # Priority: constructor > env var > default (A2)
        self.cgf_endpoint = (cgf_endpoint or Config.CGF_ENDPOINT).rstrip("/")
        self.adapter_type = adapter_type
        self.host_config = host_config or {}
        self.timeout_ms = (timeout_ms or Config.CGF_TIMEOUT_MS) / 1000  # Convert to seconds for aiohttp
        ...
        self.fail_mode_table: Dict[str, str] = {}
        # P0 Fix: Last-resort safe defaults (only used if no table cached)
        self._default_fail_modes = {
            "tool_call:high": "fail_closed",
            "tool_call:medium": "fail_closed",
            "tool_call:low": "fail_open",
            "memory_write:high": "fail_closed",
            "memory_write:medium": "fail_closed",
            "memory_write:low": "fail_open",
        }
    ...
    def _update_fail_mode_table(self, fail_mode_table: List[Dict]) -> None:
        ...
        for entry in fail_mode_table:
            ...
            self.fail_mode_table[key] = entry['fail_mode']
    
    def _get_fail_mode(self, action_type: str, risk_tier: str) -> str:
        ...
        return self._default_fail_modes.get(key, "fail_closed")
        ...
        try:
            async with session.post(
                ...
                if resp.status == 200:
                    ...
                    fail_mode_table = data.get("fail_mode_table")
                    if fail_mode_table:
                        self._update_fail_mode_table(fail_mode_table)
    ...
    def _apply_fail_mode(self, risk_tier: str, tool_name: str) -> Decision:
        """Apply fail mode from cached fail_mode_table (P0 Fix).
        
        Uses cached fail_mode_table from CGF /register response.
        Falls back to safe defaults only if no table cached.
        """
        action_type = "tool_call"  # v0.1 only supports tool_call
    ...
    def emit_event(self, event_type: str, payload: Dict) -> None:
        ...
        # P0 Fix: Include schema_version in all events
        event = {
            "event_type": event_type,
            "schema_version": "0.1.0",  # v0.1 adapter uses 0.1.0
            "adapter_id": self.adapter_id,
            "timestamp": time.time(),
            "payload": payload
        }
