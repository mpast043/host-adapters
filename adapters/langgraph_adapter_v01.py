...
class HostAdapter:
    ...
    def __init__(self, host_config: HostConfig):
        ...
        # P0 Fix: Cache fail_mode_table from CGF
        self.fail_mode_table: Dict[str, str] = {}
        # P0 Fix: Last-resort safe defaults
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if resp.status == 200:
                        data = await resp.json()
                        decision_data = data.get("decision") if isinstance(data, dict) else data
                        
                        # Handle both wrapped and unwrapped responses
                        if isinstance(decision_data, str):
                            # Server returned just the decision type string
                            decision = CGFDecision(
                                decision_id=generate_id("dec"),
                                proposal_id=proposal.proposal_id,
                                decision=DecisionType(decision_data),
                                confidence=1.0,
                                justification="Decision from CGF",
                                reason_code="CGF_RESPONSE"
                            )
            ...
            if fail_mode == "fail_closed":
                ...
                outcome = await self.observe_execution(
                    ...
                    CGFDecision(
                        ...
                        confidence=0.0,
                        justification=f"CGF unreachable - fail_mode={fail_mode} for {action_type}:{risk_tier}",
                ...
                outcome = await self.observe_execution(
                    ...
                    CGFDecision(
                        ...
                        confidence=0.0,
                        justification=f"CGF unreachable - deferred (fail_mode={fail_mode})",
                        reason_code="CGF_UNREACHABLE_DEFER"
                    ),
                    success=False,
                    duration_ms=duration_ms,
                    error=str(e)
                ...
                self.emit_event(
                    ...
                    {"reason": str(e), "proceeding_with_fail": fail_mode, "source": "fail_mode_table"}
                )
